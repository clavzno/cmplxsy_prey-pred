...
patches-own [
  ; amt-fish  ; how many fish per patch, <-- use food gain instead
...
to setup-patches
  ; initially, all patches are (blue - 1)
  ask patches [
    set pcolor (blue - 1)
    set has-fish? false
  ...
  ask n-of initial-number-food patches [
    set pcolor (grey + 3.5)
    set has-fish? true
    ; set amt-fish (random 50 + 5) <- just make it food gain
...
to setup-predator
  ; orcas start with a lower energy, they're bigger so they don't need as much energy to move around
...
to go
  ...
  ifelse (count seals >= 1 AND count orcas <= 0) [ print "togo: orcas died before seals" ]
    [ if (count orcas >= 1 AND count seals <= 0) [print "togo: seals died before orcas"] ]
  if (count seals) <= 0 or (count orcas) <= 0 [ stop ]
  if enable-stop-at-500? [ if ticks = 500 [ stop ] ]

  ask seals [
    move
    eat-fish
    if (count seals >= 1 and count orcas <= 0) [ print "askSEALS: orcas died before seals" ]
    die-if-no-energy
    reproduce-seals
    set age (age + 1)
    ; die-if-too-old
  ...
  ask orcas [
    move
    eat-seal
    if (count orcas >= 1 and count seals <= 0) [ print "askORCAS: seals died before orcas" ]
    die-if-no-energy
    reproduce-orcas
    set age (age + 1)
    ; die-if-too-old
...
to move
  ifelse follow-food? [follow-food-block] [
    ifelse (breed != orcas) [
      ; if seal, move forward, lose energy amounting to seal movement cost
      rt random-float 50
      lt random-float 50
      fd 1
      set energy (energy - seal-move-cost)
    ] [
      ; if orca, move forward, lose energy amounting to orca movement cost
      rt random-float 50
      lt random-float 50
      fd 1
...
to eat-fish
  ; seals eat the food patches
  ; seals-food-gain is a flat value <- seals become op if we make it a percentage of a flat value called food-patch-energy
  if pcolor = (grey + 3.5) [
    set pcolor (blue - 1)
    set energy (energy + seals-food-gain)
    set countdown food-respawn-time ; reset food
    set has-fish? false
...
to reproduce-seals
  ; if the amount of seals are less than max, flip a coin, then lose energy and hatch 1
...
to reproduce-orcas-old
  ; if the amount of orcas are less than max, flip a coin, then lose energy and hatch 1
...
to reproduce-orcas
  let orca-reproduction-threshold 2000 + 5000 ; the higher it is, the more energy they need to reproduce in the first place
  if (count orcas < max-orcas AND energy >= orca-reproduction-threshold) [
    if coin-flip? [
      set energy (energy - (reproduction-cost * 2)) ; they take twice as much to reproduce compared to seals
...
to eat-seal
  ; orcas eat seals and gain a percentage of the seal's energy
  ; orcas-food-gain is a percentage of the prey
  let energy-gain-cap 2000
  let prey one-of seals-here
  if prey != nobody [
    let prey-energy [energy] of prey
    if prey-energy > energy-gain-cap [
      set prey-energy 1500
    ]
    ask prey [die]
    ; current energy + a percentage of the prey
    let total-gain (prey-energy * (orcas-food-gain / 100))
    set energy (energy + total-gain)
    set food-eaten food-eaten + 1
  ]
  ; else they dont eat anything
end

to eat-seal-with-extra-nerf
    ; orcas eat seals and gain a percentage of the seal's energy
  ; orcas-food-gain is a percentage of the prey
  let energy-gain-cap 2000
  let max-orca-energy 5000
  let prey one-of seals-here
  if prey != nobody [
    let prey-energy [energy] of prey
    if prey-energy > energy-gain-cap [
    ...
    let total-gain (prey-energy * (orcas-food-gain / 100))
    set energy (energy + total-gain)
    if energy > max-orca-energy [
      set energy max-orca-energy
    ]
    set food-eaten food-eaten + 1
  ]
  ; else they dont eat anything
...
440
65
Please check Info Tab > Prerequisites before running!
15
15.0
1
...
seals-shape
seals-shape
"dot" "cylinder" "bird side" "custom-seal"
3
...
"" ""
PENS
"seals" 1.0 0 -7500403 true "" "plot count seals"
"orcas" 1.0 0 -2674135 true "" "plot count orcas"
"food" 1.0 0 -955883 true "" "plot count patches with [has-fish? = true]"
...
@#$#@#$#@
# Group 3 Prey-Predator Model: Seals and Orcas
...
true
0
Circle -7500403 true true 45 75 62
Polygon -7500403 true true 60 75 30 105 30 120 15 135 45 165 90 150 165 150 195 165 210 210 195 255 225 225 255 240 255 210 240 210 195 75 90 60
Polygon -7500403 true true 105 60 210 30 180 60 165 90
Polygon -7500403 true true 105 150 135 180 150 165 135 135
Polygon -1 true false 15 135 60 150 90 135 105 150 45 165
Polygon -1 true false 90 105 60 120 75 135 105 120
Polygon -1 true false 135 150 165 135 195 150 210 165 195 165 165 150

custom-seal
true
0
Circle -7500403 true true 41 116 67
Polygon -7500403 true true 45 135 30 165 15 180 45 195 60 195 75 180 120 210 225 195 255 210 270 240 300 180 270 180 210 120 90 120
Polygon -7500403 true true 135 210 165 225 195 225 165 195 120 195 135 210 135 210
Polygon -16777216 true false 60 150 60 150 60 165 75 150
